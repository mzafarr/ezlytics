...
**Acceptance Criteria:**
- [ ] Session dimension context = normalized values from first pageview of session.
- [ ] Dimension rollups for country/region/city/device/browser increment sessions based on session start context.
- [ ] Dimension rollup rebuild uses same rules.
- [ ] Geo conversion rate uses dimension session totals (not pageviews fallback).
...
**Acceptance Criteria:**
- [ ] Geo points filter uses event timestamp (not createdAt).
- [ ] End date is inclusive (end-of-day).
- [ ] Bot events excluded.
- [ ] Integration test validates date range boundaries.
- [ ] Typecheck/lint passes.
...
**Acceptance Criteria:**
- [ ] Goals endpoint requires an idempotency key (event_id) or uses a deterministic key.
- [ ] Payments endpoint dedupes using transaction_id (or event_id).
- [ ] Raw events for goal/payment use event_id for unique constraint.
- [ ] Retries return ok without changing rollups.
- [ ] Typecheck/lint passes.
...
**Acceptance Criteria:**
- [ ] Add golden datasets for ingest and expected rollups.
- [ ] Add property-based test: live ingest rollups == rebuild rollups for same events.
- [ ] Add reconciliation job/endpoint that diffs rollups vs raw_event rebuild (dry-run allowed).
...
**Acceptance Criteria:**
- [ ] “Visitors” and “Visitors now” show tooltip with definition from spec.
- [ ] Bounce rate and Avg session duration show definition tooltip.
- [ ] Typecheck/lint passes.
